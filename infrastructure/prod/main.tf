...
resource "aws_s3_bucket" "frontend" {
  bucket = "group-3-s3-netflix-clone-frontend-prod"
  acl    = "public-read"
...
resource "aws_ecs_cluster" "netflix_clone_prod" {
  name = "group-3-ecs-netflix-clone-prod"
}

resource "aws_ecs_task_definition" "frontend" {
  family                = "group-3-task-def-netflix-clone-frontend-prod"
  network_mode          = "awsvpc"
...
resource "aws_ecs_task_definition" "backend" {
  family                = "group-3-task-def-netflix-clone-backend-prod"
  network_mode          = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                   = 256
  memory                = 512
...
resource "aws_ecs_service" "frontend" {
  name            = "group-3-ecs-service-netflix-clone-frontend-prod"
  cluster         = aws_ecs_cluster.netflix_clone_prod.id
...
resource "aws_ecs_service" "backend" {
  name            = "group-3-ecs-service-netflix-clone-backend-prod"
  cluster         = aws_ecs_cluster.netflix_clone_prod.id
