...
resource "aws_s3_bucket" "frontend" {
  bucket = "group-3-s3-netflix-clone-frontend-stage"
  acl    = "public-read"
...
resource "aws_ecs_cluster" "netflix_clone_stage" {
  name = "group-3-ecs-netflix-clone-stage"
}

resource "aws_ecs_task_definition" "frontend" {
  family                = "group-3-task-def-netflix-clone-frontend-stage"
  network_mode          = "awsvpc"
...
resource "aws_ecs_task_definition" "backend" {
  family                = "group-3-task-def-netflix-clone-backend-stage"
  network_mode          = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                   = 256
  memory                = 512
...
resource "aws_ecs_service" "frontend" {
  name            = "group-3-ecs-service-netflix-clone-frontend-stage"
  cluster         = aws_ecs_cluster.netflix_clone_stage.id
...
resource "aws_ecs_service" "backend" {
  name            = "group-3-ecs-service-netflix-clone-backend-stage"
  cluster         = aws_ecs_cluster.netflix_clone_stage.id
